...
# 3.10 Programming Exercises — 원문 / 번역 / 정답
...
번역
3.2절의 AddTwo 프로그램을 참고하여 다음 식을 계산하는 프로그램을 작성하라:
A = (A + B) − (C + D).
EAX, EBX, ECX, EDX에 정수 값을 넣어라.

정답 / 해설
요구사항: 레지스터만 사용해 A=(A+B)−(C+D) 계산.
예시 해설(코드 아님, 논리 설명만 제공):
...
번역
일요일부터 토요일까지 상수 기호(symbolic constants)를 정의하고, 이 상수들을 초기값으로 사용하는 배열 변수를 작성하라.

정답 / 해설
요구: 월~일 숫자 또는 코드값을 기호 상수로 선언, 배열에서 사용.
예시 개념:
...
weekArr BYTE SUNDAY, MONDAY, ..., SATURDAY
...
번역
3.4절 표 3-2에 나오는 모든 데이터 타입을 하나씩 정의하고, 각각 타입에 맞는 초기값을 넣어라.

정답 / 해설
표 3-2의 데이터 타입: BYTE, SBYTE, WORD, SWORD, DWORD, SDWORD, QWORD, TBYTE 등.
각 타입에 맞는 범위의 값을 초기화해야 한다.
예: BYTE 255, SBYTE -5, WORD 1000, SDWORD -12345, QWORD 큰 양수 등.
...
번역
여러 문자열 리터럴(따옴표 안의 문자들)에 대해 기호 이름을 정의하고, 각 기호 이름을 변수 정의에 사용하라.
...
번역
AddTwoSum 프로그램에 대해 리스팅 파일을 생성하고, 각 명령에 대해 생성된 기계어 바이트를 설명하라. 일부 바이트 값의 의미는 추론해야 할 수도 있다.
...
소스 코드
가 함께 출력됨.
예를 들어 ADD EAX,5 는 05 05 00 00 00 으로 표시될 수 있고, 이는
05 = EAX에 즉값 더하기 opcode
05 00 00 00 = 즉값 5 (리틀엔디안)
이라는 의미.

(실제 리스팅 파일은 코드가 필요하므로 이 문제에서는 개념 해설로 충분)
...
번역
AddVariables 프로그램을 수정하여 64비트 변수를 사용하도록 하라. 어셈블러가 발생시키는 문법 오류와 이를 해결한 방법을 설명하라.
...
32비트 레지스터(EAX, EBX 등)는 64비트 변수 직접 로드 불가

MOV r64, qword ptr [...] 와 같이 정확한 64비트 레지스터(RAX, RBX 등) 사용 필요
...
RAX, RBX 등 64비트 레지스터로 수정

REX prefix가 필요한 명령 확인
